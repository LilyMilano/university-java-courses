...
public class MovingRects extends JPanel implements ActionListener {
    ...
    public void drawFrame(Graphics g, int frameNumber, double elapsedSeconds, int width, int height) {
        ...
        /* By using the remainder of frameNumber divided by 15, the inset value
        will cycle through the sequence of values 0, 1, 2, ..., 14 repeatedly
        as the animation progresses.

        To understand why the inset is set to the remainder of frameNumber
        divided by 15, let's consider a numeric example:

        For frame number 0, the remainder of 0 divided by 15 is 0. So, inset is set to 0.
        For frame number 1, the remainder of 1 divided by 15 is 1. So, inset is set to 1.
        For frame number 2, the remainder of 2 divided by 15 is 2. So, inset is set to 2.
        This pattern continues until frame number 14, where the remainder of 14 divided by 15 is 14. So, inset is set to 14.
        For frame number 15, the remainder of 15 divided by 15 is 0. So, inset goes back to 0, and the sequence starts repeating.
        The same pattern repeats for subsequent frames: 1, 2, ..., 14, 0, 1, 2, ..., 14, 0, and so on.

        By setting inset to the remainder of frameNumber divided by 15, we
        ensure that the inset value cycles through the desired sequence of
        values repeatedly as the animation progresses. This creates the
        effect of the rectangles moving inward towards the center of the
        drawing area.
        */

        rectWidth = width - 2*inset;
        rectHeight = height - 2*inset;
        /* The first rectangle that is drawn in a frame fills the drawing
        area except for a border of size inset around the outside of the
        rectangle. This means that the width of the first rectangle is the
        width of the drawing area minus two times the inset, and similarly
        for the height.*/

        while (rectWidth >= 0 && rectHeight >= 0) {
            g.drawRect(inset, inset, rectWidth, rectHeight);
            inset += 15;    // rectangles are 15 pixels apart
            rectWidth -= 30;    // -15px left -15 px right (the inset)
            rectHeight -= 30;    // -15px top -15px bottom
        }
    }
