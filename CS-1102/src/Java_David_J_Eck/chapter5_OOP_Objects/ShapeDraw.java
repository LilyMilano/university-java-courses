...
public class ShapeDraw extends JPanel {
    ...
    public ShapeDraw() {
        ...
        JButton rectButton = new JButton("Rect");   // buttons for adding
        // shapes.
        ...
        add(canvas, BorderLayout.CENTER);   // add canvas and controls to the
        // panel
        add(bottom, BorderLayout.SOUTH);

    }   // end of constructor  ShapeDraw()

    JComboBox<String> colorChoice; // Color selection menu

    //---- Nested class definitions ---
    //
    // The remainder of the ShapeDraw class consists of static nested class
    // definitions.
    // These are just like regular classes, except that they are defined inside
    // another class (and hence have full names, when used outside this
    // class, such as ShapeDraw.ShapeCanvas).

    /**
     * This nested class defines the drawing area.
     */
    class DrawingArea extends JPanel implements ActionListener, MouseListener
            , MouseMotionListener {
        ...
        public void paintComponent(Graphics g) {
            // In the paint method, all the shapes in ArrayList are copied
            // onto the canvas.
            g.setColor(Color.WHITE);
            g.fillRect(0, 0, getSize().width, getSize().height);
            for (int i = 0; i < shapeCount; i++) {
                Shape s = shapes[i];
                s.draw(g);  // Polymorphic method
            }
            g.setColor(Color.BLACK);    // Draw a black border around the
            // edge of the drawing area.
            g.drawRect(0, 0, getWidth() - 1, getHeight() - 1);
        }   // end of paintComponent()

        public void actionPerformed(ActionEvent event) {
            // Call to respond to action events. The three shape-adding
            // buttons have been set up to send action events to the canvas.
        ...
        void addShape(Shape shape) {
            ...
            // The shape is added in the top-left corner, with size 80-by-50.
    ...
    static abstract class Shape {
        ...
        int left, top;  // Position at the left corner of rectangle that
        // bounds this shape.
        ...
        void moveBy(int dx, int dy) {
            // Move the shape by dx pixels horizontally and dy pixels
            // vertically (by changing the position of the top-left corner of
            // the shape).
            left += dx;
            top += dy;
        }
    ...
    }   // end of static class RoundRectShape

}   // END OF CLASS ShapeDraw
